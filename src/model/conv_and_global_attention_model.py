...
class GlobalAttention(keras.layers.Layer):
    ...
    def get_config(self):
        ...
        config.update({"n_heads":self.n_heads,"d_key":self.d_key,"d_value":self.d_value})
        return config
    ...
    def call(self, inputs):
        ...
        # X: (batch_size, d_global_input) global
        # S: (batch_size, length, d_seq_input) sequence
        X, S = inputs
...
def create_model(seq_len, vocab_size, n_annotations, d_hidden_seq = 128, d_hidden_global = 512, n_blocks = 6, n_heads = 4, \
    ...
    for block_index in range(1, n_blocks + 1):
        ...
        attention = GlobalAttention(n_heads, d_key, d_value, name = 'global-attention-block%d' % block_index)([hidden_global, hidden_seq]) #n_heads = 4, d_key = 64, d_value = 128
