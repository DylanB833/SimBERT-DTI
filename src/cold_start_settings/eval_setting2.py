...
settings = {
    ...
    'L1': 0.5, # Weight for Drug Similarity
    'L2': 0.0001, # Weight for Protein Similarity
...
def run_benchmark(benchmark_name, pretraining_model_generator, input_encoder, pretraining_model_manipulation_function = None):
    ...
    train_set, valid_set, test_set, train_sims, val_sims, label_sims, cmask, tcmask = load_benchmark_dataset(benchmark_name)
    ...
    output_spec = OutputSpec(output_type, unique_labels, global_labels, train_sims = train_sims, val_sims = val_sims, label_sims = label_sims, col_holdouts = cmask, test_holdouts = tcmask, L1=settings['L1'], L2=settings['L2'])
...
def load_benchmark_dataset(benchmark_name):
    ...
    if settings['use_similarity']:
            ...
            cmask_path = os.path.join(BENCHMARKS_DIR, "%s_labelmask.csv" % benchmark_name)
            tcmask_path = os.path.join(BENCHMARKS_DIR, "%s_labelmask.csv" % benchmark_name)
            label_sims = pd.read_csv(label_path).values
            train_sims = pd.read_csv(tsims_path).values
            val_sims = pd.read_csv(vsims_path).values
            cmask = pd.read_csv(cmask_path).to_numpy().flatten().tolist()
            tcmask = pd.read_csv(tcmask_path).to_numpy().flatten().tolist()
    else:
            label_sims = None
            train_sims = None
            val_sims = None

    return train_set, valid_set, test_set, train_sims, val_sims, label_sims, cmask, tcmask
