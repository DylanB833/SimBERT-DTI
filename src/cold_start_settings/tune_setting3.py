...
#BENCHMARKS_DIR = '/home/bockdaa/protein_bert/protein_benchmarks' #original benchmark directory

#BENCHMARKS_DIR = '/home/bockdaa/protein_bert/my_benchmarks' #new benchmark directory

#BENCHMARKS_DIR = '/home/bockdaa/FinalProject/ProjectBenchmarks' #TTD and Biosnap directory

BENCHMARKS_DIR = '/home/bockdaa/drugbank_files/round1s3' # New directory containing similarity matrices
...
#    ('AdhesionData', OutputType(False,'binary')),
#    ('numericSequence',OutputType(True,'numeric')),
    ('Drugbank', OutputType(False,'binary',True)),
#    ('PSICGPCR',OutputType(False,'binary',True)),
#    ('HPIDB',OutputType(False,'binary',True)),
#    ('TTD',OutputType(False,'binary',True)),
#    ('Biosnap',OutputType(False,'binary',True)),
...
def run_benchmark(benchmark_name, pretraining_model_generator, input_encoder, pretraining_model_manipulation_function = None, fold=0):
    ...
    train_set, valid_set, test_set, train_sims, val_sims, label_sims, hmask = load_benchmark_dataset(benchmark_name,fold)
    ...
    output_spec = OutputSpec(output_type, unique_labels, global_labels, train_sims = train_sims, val_sims = val_sims, label_sims = label_sims, holdouts=hmask, L1=settings['L1'], L2=settings['L2'])
...
def load_benchmark_dataset(raw_benchmark_name, fold):
    ...
    test_set_file_path = valid_set_file_path # Use the validation set as the test set during cross-validation to ensure test set stays "blind". Only run real test set once hyperparameters are finetuned.
    ...
    if settings['use_similarity']:
            ...
            hmask_path = os.path.join(BENCHMARKS_DIR, "%s_holdouts.csv" % benchmark_name)
            label_sims = pd.read_csv(label_path).values
            train_sims = pd.read_csv(tsims_path).values
            val_sims = pd.read_csv(vsims_path).values
            hdf = pd.read_csv(hmask_path)
            hmask = list(zip(hdf['row'],hdf['label']))
    else:
            label_sims = None
            train_sims = None
            val_sims = None

    return train_set, valid_set, test_set, train_sims, val_sims, label_sims, hmask
...
for fold in range(1,6):
    ...
    with open(f"S3_Fold_{fold}.csv","w",newline="") as f:
            writer = csv.writer(f)
