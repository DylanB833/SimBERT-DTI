...
def run_benchmark(benchmark_name, pretraining_model_generator, input_encoder, pretraining_model_manipulation_function = None, fold=0):
    ...
    train_set, valid_set, test_set, train_sims, val_sims, label_sims, cmask = load_benchmark_dataset(benchmark_name,fold)
    ...
    output_spec = OutputSpec(output_type, unique_labels, global_labels, train_sims = train_sims, val_sims = val_sims, label_sims = label_sims, col_holdouts = cmask, L1=settings['L1'], L2=settings['L2'])
...
def load_benchmark_dataset(raw_benchmark_name, fold):
    ...
    return train_set, valid_set, test_set, train_sims, val_sims, label_sims, cmask
...
for fold in range(1,6):
    ...
    with open(f"S2_Fold_{fold}.csv","w",newline="") as f:
            writer = csv.writer(f)
