...
settings = {
    ...
    'L1': 0.0001, # Weight for Drug Similarity
    'L2': 0.003, # Weight for Protein Similarity
...
def run_benchmark(benchmark_name, pretraining_model_generator, input_encoder, pretraining_model_manipulation_function = None):
    ...
    train_set, valid_set, test_set, train_sims, val_sims, label_sims = load_benchmark_dataset(benchmark_name)
...
def load_benchmark_dataset(benchmark_name):
    ...
    if settings['use_similarity']:
            ...
            train_sims = pd.read_csv(tsims_path,header=None).values
            val_sims = pd.read_csv(vsims_path,header=None).values
    else:
            label_sims = None
            train_sims = None
            val_sims = None
